...
def scatter_top(amp_range_f: List[float] = None, seq_y_vals: List[int] = None):
    _df = set_df_f()
    _df = filter_data(_df, amp_range_f, seq_y_vals)

    scatter_fig = px.scatter(
        _df,
        x="r12_load_MW",
        y="dda50FrequencyHz",
        title="Sensor response vs Frequency&Load",
        # colorscale="Inferno",
        # showscale=True
        hover_data=[
            "Tex_F",
...
def scatter_1(amp_range_f: List[float] = None, seq_y_vals: List[int] = None):
    ...
    fig.update_layout(title_text="Load / Speed / Amplitude in time")
    ...
    return dcc.Graph(id="scatter-1", figure=fig)
